...
jest.mock("../../components/UserMenu", () => () => <div data-testid="user-menu-mock">UserMenu</div>);
...
describe('Profile Component', () => {
  ...
  it('should hydrate form fields from context and enforce email disability', async () => {
    ...
    await waitFor(() => expect(getByDisplayValue("Initial Name")).toBeInTheDocument());
    expect(getByDisplayValue("initial@test.com")).toBeInTheDocument();
    expect(getByPlaceholderText(/enter your email/i)).toBeDisabled();
    expect(getByTestId('layout-mock')).toHaveAttribute('data-title', 'Your Profile');
  });

  it('should update state for all editable fields', async () => {
    const { getByPlaceholderText } = render(<MemoryRouter><Profile /></MemoryRouter>);
  ...
  it('should verify axios.put payload and storage sync on success', async () => {
    ...
    // Wait for hydration before acting
    await waitFor(() => expect(getByDisplayValue("Initial Name")).toBeInTheDocument());

    fireEvent.change(getByPlaceholderText(/enter your name/i), { target: { value: 'John Updated' } });
    fireEvent.click(getByText('UPDATE'));

    await waitFor(() => expect(axios.put).toHaveBeenCalledWith("/api/v1/auth/profile", expect.objectContaining({
      name: "John Updated"
    })));

    expect(mockSetAuth).toHaveBeenCalled();
    expect(localStorage.setItem).toHaveBeenCalledWith("auth", expect.stringContaining("John Updated"));
  ...
  it('should handle logical error and network failure correctly', async () => {
    ...
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith("Update failed logic"));
